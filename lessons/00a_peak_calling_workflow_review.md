...
Next, we take our reads and map them to genome. There are a few difficulties that alignment algorrithms need to overcome:
...
There are a variety of tools to align reads to a reference genome, but we use [Bowtie2](https://bowtie-bio.sourceforge.net/bowtie2/index.shtml) for this task. The output from this tool will be a SAM/BAM file. If you are aligning to a high-quality reference genome (human/mouse/*Drosophila*), you should expect to see an alignment rate above 90%. If your alignment dips too far below this threshold, it could be the result of contamination. 
